...
st.header("3) Waterway Profile & Penstock Geometry")
...
st.header("4) Head Loss of Hydraulic System")
...
if preset == "None":
    ...
    # Structural logic for height-to-width ratio
...
k_ratio = st.slider("K-ratio = σh / σv", 0.5, 2.0, 1.0, 0.1)
...
st.metric("Estimated Horizontal Stress σₕ", f"{sigma_h:.2f} MPa")

# Stability warnings
if H_hall / B_hall > 3.0:
    st.warning("Hall height/span ratio > 3.0 → instability risk.")
if cover_depth < 2 * B_hall:
    st.warning("Cover depth < 2 × span → risk of insufficient rock cover.")
...
if th_mode == "Auto-calculate":
    N_transformers = st.number_input("Number of Transformers", value=N_units, step=1)
    ...
    L_trans = N_transformers * 12 + 10  # 12 m per bay + 10 m clearance
else:
    B_trans = st.number_input("Transformer Hall Width Bₜ (m)", value=15.0, step=1.0)
    H_trans = st.number_input("Transformer Hall Height Hₜ (m)", value=15.0, step=1.0)
